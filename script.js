/***** GITHUB SETING *****/
const GITHUB_REPO = "Rafka-Developer/dataweb"; // ganti : owner/repo contoh : Kalzz/data
const DATA_FILE = "data.json"; //ganti pake file DB lu contoh : database.json
const GITHUB_TOKEN = "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"; // ganti : token github lu contoh : ghp_XXXXX
...
function renderAccounts(){
  ...
  db.accounts.forEach(acc=>{
    ...
    chg.onclick = () => {
      if (me.role !== "admin") return showModal("Hanya admin yang boleh.", "alert");
      showModal(`Ubah sandi untuk "${acc.username}"`, "prompt", async (yes, val)=>{
        ...
        if (target){ target.password = val; await saveDB("Change password"); showModal("Password diubah.", "alert"); }
      }, "Password baru");
    };
    ...
    del.onclick = () => {
      if (me.role !== "admin") return showModal("Hanya admin yang boleh.", "alert");
      if (acc.username === "Kalzz") return showModal("Akun Kalzz (Owner) tidak boleh dihapus.", "alert");
    ...
    if (acc.username !== "admin") actions.appendChild(del);
...
document.getElementById("addAccountForm").onsubmit = async (e) => {
  e.preventDefault();
  if (!me || me.role !== "admin") return showModal("Hanya admin yang boleh.", "alert");
  ...
  showModal(`Buat akun user baru "${u}"?`, "confirm", async (yes)=>{
    if (!yes) return;
    db.accounts.push({ username: u, password: p, role: "user" });
    await saveDB("Create account");
    document.getElementById("newUser").value = "";
    document.getElementById("newPass").value = "";
    renderAccounts();
    renderDashboard();
    showModal("✅ Akun berhasil dibuat!", "alert");
  });
};
